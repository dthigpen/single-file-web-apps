...
function TokenRow({ item }) {
    ...
    const formattedToken = useComputed(() =>
        activeToken.value.length === 6
            ? `${activeToken.value.slice(0, 3)} ${activeToken.value.slice(3)}`
            : activeToken.value
    );
...
function TotpAppRoot() {
    ...
    const processTextTokens = (textDataString) => {
        ...
        if (
            confirm(
                `Parsed ${structuralItems.length} secret tokens successfully. Append them into your current active profile database storage context?`
            )
        ) {
            ...
            <div class="card" style="margin-top: 4px;">
                ...
                <form onSubmit=${(e) => {
                    ...
                    if (!newLabel.value.trim() || !newSecret.value.trim()) {
                        formError.value = 'Fields required.';
                        return;
                    }
